...
__global__ void gemm(float *a, float *b, float *c,const float alpha,
        const float beta, float *output, const int input_size){
    ...
    for(int p = 0; p < input_size/TILE_WIDTH; ++p){
        ...
        if(row==(input_size-1)&&((row+1)%TILE_WIDTH!=0)){
            for(int i=1; (row+i)%TILE_WIDTH!=0; ++i){
            if((col+((p*TILE_WIDTH+ty+i)*input_size))>=input_size*input_size) continue;
                s_b[ty+i][tx]=b[col+((p*TILE_WIDTH+ty+i)*input_size)];
            }
        }
        if((p*TILE_WIDTH+ty)<(input_size)&&col<input_size){
            s_b[ty][tx]=b[col +((p*TILE_WIDTH+ty)*input_size)];
        }else{
            s_b[ty][tx]=0.0;
        }
